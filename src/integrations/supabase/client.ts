  ...
  try {
    ...
    try {
      ...
      if (!createProfilesError && newProfiles && newProfiles.length > 0) {
        ...
        // Create roles for these profiles with proper type casting
        const roleInserts = newProfiles.map((profile, index) => ({
          user_id: profile.id,
          role: (index === 0 ? 'admin' : (index === 1 ? 'manager' : 'user')) as 'admin' | 'manager' | 'user'
        }));
        ...
        if (newRoles) {
          ...
          return newProfiles.map((profile, index) => ({
            ...profile,
            user_roles: { role: index === 0 ? 'admin' as const : (index === 1 ? 'manager' as const : 'user' as const) }
          }));
        }
    ...
    return [
      ...
      },
      {
        ...
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
        user_roles: { role: 'manager' as const }
      },
      {
...
// Fix the getUserCount function to properly handle errors
...
// Make sure the getDetailedAppointments function fetches all existing appointments first
